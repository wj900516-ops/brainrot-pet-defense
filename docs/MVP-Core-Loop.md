...
| 技能 UI | `StarterGui/SkillTreeDebug/SkillTreeDebug.client.lua`（Phase 16B） | 极简技能树调试 UI（独立 ScreenGui，不改 MainUI） | Net |
...
- S→C `"State", { skillPoints, totalPoints, unlocked, skills }`（加入时单发）；`"Result", { success, reason, skillId?, rank?, skillPoints? }`。详见 [`Phase16B-SkillTreeMVP.md`](Phase16B-SkillTreeMVP.md)
...
5. **玩家技能树（Phase 16A 设计 / 16B 最小实现）**：用 Phase 15 的 `SkillPoints` 投资 4 大分支（Economy/Tower/Pet/Defense）。
   ...
   **Phase 16B 已实现首个闭环**（详见 [`Phase16B-SkillTreeMVP.md`](Phase16B-SkillTreeMVP.md)）：持久化 `SkillTree`（`CURRENT_DATA_VERSION` 3→4，内嵌 `SkillTree.Version`，只存 `[skillId]=rank`，派生量动态计算）、
   服务端 `SkillTreeService`（去抖+锁的权威消费）+ `SkillEffectResolver`（缓存修正量）+ `SkillRemote` + 极简 `SkillTreeDebug` UI。
   **已启用 3 个技能**：`eco_kill_coins`（击杀金币 +5%/级）、`twr_damage`（塔伤害 +5%/级）、`pet_damage`（宠物伤害 +5%/级）；其余节点配置可见但不可消费。
   仍**未做**：完整树 UI / 全部效果 / Defense / Keystone / respec / 宠物技能树。
