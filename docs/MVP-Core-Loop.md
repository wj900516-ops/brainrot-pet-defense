...
| 塔 | `ServerScriptService/Services/TowerService.lua`（Phase 11） | 服务端校验+放置占位塔（花费/距路径/距塔） | PlayerDataService, EnemyService, TowerConfig |
...
`TowerRemote`（Phase 11）
- C→S `"PlaceTower"`（仅意图，无负载）→ 服务端读角色位置 + 校验（金币/距路径/距塔）后放置
- S→C `"Result", { success, reason, cost? }` → 客户端显示放置反馈。详见 [`Phase11-TowerPlacement.md`](Phase11-TowerPlacement.md)
