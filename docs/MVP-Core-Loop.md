...
5. **玩家技能树（设计中，Phase 16A）**：用 Phase 15 的 `SkillPoints` 投资 4 大分支（Economy/Tower/Pet/Survival）的永久强化。
   当前仅有**纯数据脚手架** [`SkillTreeConfig.lua`](../src/ReplicatedStorage/Config/SkillTreeConfig.lua)（**无人 require，不影响玩法**）
   与设计规格 [`Phase16A-SkillTreeDesign.md`](Phase16A-SkillTreeDesign.md)。Phase 16B 才接入：持久化 `SkillTree`（拟 `CURRENT_DATA_VERSION` 3→4 + 独立 `SkillTreeVersion` 标记）、
