...
6. **视觉 / UX 大改造规划（Phase 17A）**：核心系统已稳定，后续视觉/UI/模型/地图改造遵循"外观可换、服务端权威不动"的规范。
   见 [`Phase17A-VisualUXOverhaulPlan.md`](Phase17A-VisualUXOverhaulPlan.md)：受保护核心清单、视觉替换四原则、地图/PathNodes 规则、塔/宠物/敌人"视觉-逻辑分离"、
   技能树 UI 规则、未来 `modelId`/`icon` 配置字段提案，以及 17B（地图+PathNodes 资产规则）→18（新塔/解锁）→19（宠物技能树）→20（UI/美术大改）路线。
   **本阶段仅规划文档，不改任何 gameplay 代码 / DataStore / remote / 受保护文件。**
